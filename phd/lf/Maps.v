...
Inductive Value (av : Type) : Type :=
| SCA : nat -> Value av
| ADT : av -> Value av.

Check Value.

Inductive NameTag (av : Type) T :=
| NTNone : NameTag av T
| NTSome (key: string) (H: FacadeWrapper (Value av) T) : NameTag av T.

Inductive Telescope (av: Type) : Type :=
| Nil : Telescope av
| Cons : forall T (key: NameTag av T) (val:list T) 
(tail: T -> Telescope av), Telescope av.

Definition tonil (a : nat) : Telescope bool := (Nil bool) . 

Check Nil.
Check (Cons bool nat (NTNone bool nat) [1] tonil) .