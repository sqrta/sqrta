...
*)
Check forall n, even n -> even (S (S n)).
...
Proof.
  ...
  - intros.
    induction H.
    + apply even'_0.
    + assert (I:forall n:nat, n + 2 = S (S n)).
      * intros. {induction n0.
        - reflexivity.
...
Proof.
  ...
  induction H.
  - apply le_n.
  - rewrite -> IHle. apply le_S. apply le_n.
  (* FILL IN HERE *)Qed.
...
Proof.
 ...
 split.
 - induction m.
  + rewrite <- H. apply n_le_m__Sn_le_Sm. apply le_plus_l.
  + rewrite <- H. apply n_le_m__Sn_le_Sm. apply le_plus_l.
- induction m.
  + inversion H.
...
Proof.
  ...
  - intros.
    ...
    + induction n.
      * simpl. intros. rewrite -> H. apply c2. 
        apply IHo. reflexivity.
      * intros. simpl in H. injection H. intros.
        apply c3. apply IHo. apply H0.
    Qed.
...
Theorem app_concat: forall T (x:T) (s: list T),
  x::s = [x] ++ s .
Proof.
  intros.
  simpl.
...
Proof.
  ...
  - intros.
    ...
    + intros. simpl in H. inversion H.
      apply IHs2 in H4. inversion H3. rewrite -> H4.
      reflexivity.
  ...
  - intros.
    ...
    + intros. simpl. destruct s1.
      * discriminate H.
      * inversion H. rewrite -> app_concat.
        {apply MApp.
          - apply MChar.
...
Fixpoint re_not_empty {T : Type} (re : @reg_exp T) : bool :=
  match re with
    ...
    | EmptyStr => true
    | Char x => true
    | App s1 s2 => andb (re_not_empty s1) (re_not_empty s2)
    | Union s1 s2 => orb (re_not_empty s1) (re_not_empty s2)
    | Star s => true
    end.
...
Proof.
  ...
  - intros [s h]. 
    ...
    + reflexivity.
    + reflexivity.
    + simpl. rewrite IHh1. rewrite IHh2. reflexivity.
    + simpl. rewrite IHh. reflexivity.
    + simpl. rewrite IHh. destruct (re_not_empty re1). 
  ...
  - intros.
    ...
    + simpl in H. 
      ...
      apply andb_true_elim2 in H2.
      apply IHre2 in H2. destruct H2.
      exists (x ++ x0). apply MApp.
        * apply H.
        * apply H0.
    + simpl in H. destruct (re_not_empty re1).
      * assert (I: true = true). reflexivity.
        apply IHre1 in I. destruct I. exists (x).
        apply MUnionL. apply H0.
      * simpl in H. apply IHre2 in H. destruct H.
...
Proof.
  ...
  - destruct IHexp_match2.
    + apply Heqre'.
    + inversion H1. exists (s1 :: x). split.
      * rewrite -> H2. reflexivity.
      * simpl. intros. {
        destruct H4.
        - inversion Heqre'. rewrite <- H4. rewrite <- H6. apply H.
        - apply H3. apply H4.
      }
  (* FILL IN HERE *) Qed.
(** [] *)
...
Proof.
  ...
  - split.
    ...
    + intros. apply H0.
  - split.
    + intros. apply H0 in H2. destruct H2.
    + intros. discriminate.
    Qed.
  (* FILL IN HERE *) 
(** [] *)
...
Proof.
  ...
  - simpl. destruct (eqbP n x).
    ...
    + intros. simpl in H0. destruct H1.
      * symmetry in H1. unfold not in H. 
