...
Proof.
  ...
  try (unfold optimize_0plus_b; simpl;
...
Inductive bevalR: bexp -> bool -> Prop :=
  ...
  | E_BNot (b : bexp) (nb : bool): (bevalR b nb) -> 
              bevalR (BNot b) (negb nb)
  | E_BAnd (b1 b2 : bexp) (r1 r2 : bool) : 
    (bevalR b1 r1) -> (bevalR b2 r2) -> 
      bevalR (BAnd b1 b2) (andb r1 r2)
  .
...
Proof.
  ...
  - induction b; try (discriminate).
    ...
    * simpl in IHcontra2. apply IHcontra2. reflexivity.
    Qed.
  (** 
...
Inductive no_whilesR: com -> Prop :=
  ...
  | nw_seq : forall c1 c2, no_whilesR c1 -> no_whilesR c2 -> 
              no_whilesR (c1;;c2)
  | nw_test : forall c1 c2 t, no_whilesR c1 -> no_whilesR c2 -> 
          no_whilesR (TEST t THEN c1 ELSE c2 FI) .
 (* FILL IN HERE *)
...
Proof.
  ...
  - intros.
    ...
    try (constructor);
    try (inversion H);
    try (inversion H; destruct (no_whiles c1);
          [apply IHc1; reflexivity |
          simpl in H1; discriminate]
    );
    try (inversion H; destruct (no_whiles c2);
      [apply IHc2; reflexivity | destruct (no_whiles c1); discriminate]
    ).
  - intros.
    induction c; simpl; try (reflexivity);
    inversion H;
    try (apply IHc1 in H2; apply IHc2 in H3;
