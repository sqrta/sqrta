...
Fixpoint eqblist (l1 l2 : natlist) : bool :=
  match l1 with
    ...
    | h::t => match l2 with
      | [] => false
      | h'::t' => if h=?h' then eqblist t t' else false
      end
    end.
...
Proof.
  ...
  - reflexivity.
  - destruct n as [|n']
    + simpl. simpl. rewrite -> leb_n_Sn. reflexivity.
    + simpl. rewrite -> h. reflexivity.
    Qed.
  (* FILL IN HERE *)
(** [] *)
...
(* FILL IN HERE *)
Theorem rev_more: forall (l1 l2 :natlist), l1 = l2 -> rev l1 = rev l2.
Proof.
  intros l1 l2 h.
  rewrite -> h.
  reflexivity.
...
Proof.
  intros l1 l2 h.
   simpl. rewrite <- rev_involutive. rewrite <- h. 
